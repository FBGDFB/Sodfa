...
"use strict";
/* ═══════════════════════════════════════════════════════════════
SODFA — السكربت الرئيسي (النسخة المعيارية)
الإصدار: 2.1.1  |  تاريخ التحديث: 2026-08-16
───────────────────────────────────────────────────────────────
[محدّث 2.1.1]
• تأثير الأزهار/الأوراق المتساقطة أصبح طبقة عالمية واحدة (fixed)
  تغطي الصفحة كاملة من الـHero حتى الفوتر:
  - البذر تم نقله من INIT.hero إلى seedFallZone() تُستدعى مرة واحدة عند الإقلاع.
  - updateFallZoneClip() تقصّ الطبقة عند أعلى الفوتر أثناء التمرير
    فتنتهي الجسيمات عند الفوتر بشكل طبيعي ولا تتجاوزه.
[محدّث 2.1.0]
• قسم الزيوت: أول 4 زيوت + زر "إظهار المزيد من الزيوت" بتوسيع سلس.
[المرجع] النسخة الأصلية: test.24.html (إصدار 1.24)
═══════════════════════════════════════════════════════════════ */
var SODFA_VERSION = "2.1.1";
var CONFIG = null;
...
var toastOn = true;
/* عدد الزيوت الظاهرة في الحالة المطوية */
var OILS_COLLAPSED_COUNT = 4;
/* ─── أدوات مساعدة ─── */
function qs(sel, root) { return (root || document).querySelector(sel); }
function qsa(sel, root) { return Array.prototype.slice.call((root || document).querySelectorAll(sel)); }
function pad2(n) { return (n < 10 ? "0" : "") + n; }
/* ─── أدوات آمنة — لا تنكسر أبداً عند غياب عنصر ─── */
function setText(el, v) { if (el && v != null) el.textContent = v; }
function setHTML(el, v) { if (el && v != null) el.innerHTML = v; }
function setSrc(el, v) { if (el && v) el.src = v; }
function fillList(root, html) { var el = qs("[data-list]", root); if (el) el.innerHTML = html; }
...
/* ─── تحميل ملف قسم: مهلة + إعادة محاولة + تخزين مؤقت + نسخة احتياطية ─── */
...
return fetchWithTimeout(cacheBustUrl(s.file)).then(function (txt) {
...
return new Promise(function (resolve, reject) {
setTimeout(function () { fetchSection(s, attempt + 1).then(resolve, reject); }, 400 * (attempt + 1));
});
}
...
fetchSection(s).then(function (html) {
...
applyButtonSettings();
bindGlobalUI();
/* بعد اكتمال بناء الأقسام (وظهور الفوتر): حدّثي قصّ طبقة الأزهار */
updateFallZoneClip();
});
}
...
/* ═══════════════════════════════════════════════════════════════
[2.1.1] الأزهار/الأوراق المتساقطة — طبقة عالمية واحدة لكل الصفحة
───────────────────────────────────────────────────────────────
• seedFallZone(): تبث الجسيمات مرة واحدة فقط (لا تتكرر لكل قسم).
• updateFallZoneClip(): تقصّ ارتفاع الطبقة عند أعلى الفوتر أثناء
  التمرير، فتنتهي الجسيمات عند الفوتر ولا تتجاوزه أبداً.
═══════════════════════════════════════════════════════════════ */
...
/* تنويع طبيعي: حجم/سرعة/موضع/دوران/شفافية مختلفة لكل جسيم */
for (var i = 0; i < 22; i++) {
...
/* قصّ الطبقة عند أعلى الفوتر: عندما يدخل الفوتر نافذة العرض نُنهِي
طبقة الجسيمات عند حافته العليا فيختفي التساقط فوقه بشكل طبيعي. */
...
fillList(h, c.testimonials.map(function (t, index) {
var rating = (t.rating != null) ? t.rating : t.stars;
return '<div class="tst-slide"><div class="tst-card"><span class="quote">”</span><div class="tst-stars">' + starMarkup(rating, 15, "rev-" + index) + "</div><p>" + t.text + "</p>" +
...
var INIT = {
hero: function (h) {
/* [2.1.1] بذر الأزهار تم نقله إلى seedFallZone() العالمية عند الإقلاع —
لا شيء يُبث هنا حتى لا يتكرر التأثير أو يرتبط بقسم الـHero. */
/* زر النزول: يمرّر إلى أول قسم مفعّل بعد الواجهة */
...
flash: function (h, c) {
safeCountdown({
d: qs('[data-fs="d"]', h), h: qs('[data-fs="h"]', h), m: qs('[data-fs="m"]', h), s: qs('[data-fs="s"]', h)
...
var state = { visible: 0, current: 0, loopStart: 0, loopEnd: 0, timer: null, orig: 0 };
function perView() { return window.innerWidth >= 1024 ? 3 : (window.innerWidth >= 640 ? 2 : 1); }
function getSlideWidth() {
var slide = qs(".tst-slide", track);
return slide ? slide.getBoundingClientRect().width : (viewport.clientWidth / Math.max(perView(), 1));
}
function buildLoop() {
var slides = qsa(".tst-slide", track);
if (!slides.length) return;
state.orig = slides.length;
state.visible = perView();
var cloneFront = [];
var cloneBack = [];
for (var i = 0; i < state.visible; i++) {
cloneFront.push(slides[(slides.length - 1 - i + slides.length) % slides.length].cloneNode(true));
cloneBack.push(slides[i % slides.length].cloneNode(true));
}
track.innerHTML = "";
cloneFront.forEach(function (node) { track.appendChild(node); });
slides.forEach(function (slide) { track.appendChild(slide.cloneNode(true)); });
cloneBack.forEach(function (node) { track.appendChild(node); });
state.loopStart = state.visible;
state.loopEnd = state.visible + state.orig - 1;
state.current = state.loopStart;
apply(false);
}
function apply(anim) {
if (anim === false) track.classList.add("noanim");
track.style.transform = "translate3d(" + (-state.current * getSlideWidth()) + "px, 0, 0)";
if (anim === false) { void track.offsetWidth; track.classList.remove("noanim"); }
}
function move(dir) {
if (dir > 0) {
state.current += 1;
if (state.current > state.loopEnd) state.current = state.loopStart;
} else {
state.current -= 1;
if (state.current < state.loopStart) state.current = state.loopEnd;
}
apply();
}
function resetTimer() {
if (state.timer) clearInterval(state.timer);
state.timer = setInterval(function () { move(1); }, 4000);
timers.push(state.timer);
}
next.addEventListener("click", function () { move(1); resetTimer(); });
prev.addEventListener("click", function () { move(-1); resetTimer(); });
viewport.addEventListener("mouseenter", function () { if (state.timer) clearInterval(state.timer); });
...
track._onResize = function () {
var newVisible = perView();
...
if (bellFab && bellPanel && tPanel) bellFab.addEventListener("click", function () { bellPanel.classList.toggle("open"); tPanel.classList.remove("open"); });
var bellClose = qs("#bellClose"); if (bellClose && bellPanel) bellClose.addEventListener("click", function () { bellPanel.classList.remove("open"); });
var openThemeFromBell = qs("#openThemeFromBell"); if (openThemeFromBell && tPanel) openThemeFromBell.addEventListener("click", function () { bellPanel.classList.remove("open"); tPanel.classList.add("open"); });
...
pageState = defs.page.map(function (s) { var o = {}; for (var k in s) o[k] = s[k]; return o; });
...
if (tFab && tPanel) tFab.addEventListener("click", function () { tPanel.classList.toggle("open"); bellPanel.classList.remove("open"); });
var tpClose = qs("#tpClose"); if (tpClose && tPanel) tpClose.addEventListener("click", function () { tPanel.classList.remove("open"); });
...
/* ─── التنقل والقوائم + قصّ طبقة الأزهار عند الفوتر أثناء التمرير ─── */
...
/* [2.1.1] تحديث خفيف (rAF) لقصّ الطبقة عند أعلى الفوتر */
if (!fzClipTick) {
fzClipTick = true;
requestAnimationFrame(function () { fzClipTick = false; updateFallZoneClip(); });
}
}
...
if (burger && mMenu) burger.addEventListener("click", function () { burger.classList.toggle("open"); mMenu.classList.toggle("open"); });
if (mMenu) qsa("a,button", mMenu).forEach(function (a) { a.addEventListener("click", function () { burger.classList.remove("open"); mMenu.classList.remove("open"); }); });
...
console.info("SODFA v" + SODFA_VERSION + " — النسخة المعيارية المحدّثة (2026-08-16). المرجع: test.24.html");
document.documentElement.setAttribute("data-sodfa-version", SODFA_VERSION);
applySeo();
bindSiteLogos(document, CONFIG);
/* [2.1.1] بذر طبقة الأزهار العالمية مرة واحدة + القص الابتدائي */
seedFallZone();
updateFallZoneClip();
